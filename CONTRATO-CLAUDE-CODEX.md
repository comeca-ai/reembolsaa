...
- _(Claude)_ **Fase 4 — mocks ESCONDIDOS** (decisão do usuário): flag `src/lib/features.js` (`MOCK_MODULES_ENABLED=false`). Financeiro/Relatórios/Detalhe de Alerta saíram do menu (`Sidebar`/`MobileNav` via `visibleNavItems`) e as rotas redirecionam p/ `/dashboard` (App.jsx). Reativar = flag true (ou integrar de verdade). **Codex:** ao integrar esses módulos ao backend real, é só reativar o flag.
- _(Codex)_ …
